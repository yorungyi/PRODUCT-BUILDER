...
1. **사용자 인증 시스템**
   ...
   - 관리자/직원 권한 구분
   - 세션 관리 (7일 유효기간)
   - **비밀번호 변경 기능** (관리자/직원 모두)

2. **일매출 등록**
   - 날짜별, 점포별 매출 등록
   - **날씨 선택** (☀️맑음, ☁️흐림, 🌧️비, ❄️눈, 🚫휴장)
   - **숫자 키패드** 형식 매출액 입력 (모바일 최적화)
   - 메모 기능 (특이사항 기록)
   - 중복 등록 방지 (같은 날짜+점포)
   - 실시간 입력 검증
...
5. **대시보드**
   - 점포별 매출 합계 (카드 형식)
   - **당월 총매출** 실시간 표시 (기간별 조회 우측, 파란색 강조)
   - 점포별 매출 비중 (도넛 차트)
   - **당월 일별 매출 추이** (세로 막대 그래프, 4개 점포 구분)
   - 기간별 조회 (날짜 범위 선택)

6. **매출 내역 조회**
   - 전체 매출 목록 (테이블 형식)
   - **필터 적용 시 항상 총매출 표시** (점포별/기간별 필터 적용 시에도 자동 계산)
   - **당일 총매출 박스** (하루 조회 시, 녹색 그라데이션)
   - **당월/기간별 총매출 박스** (여러 날짜 조회 시, 파란색 그라데이션)
   - 총매출 = 순매출 + 부가세 (자동 계산)
   - **날씨 아이콘** 표시 (☀️☁️🌧️❄️🚫)
   - 다중 필터 (점포, 마감여부, 날짜범위)
   - 마감 상태 표시 (배지)
...
- `POST /api/sales` - 매출 등록 (날씨 포함)
  - Body: `{ saleDate, storeId, amount, memo, weather }`
- `PUT /api/sales/:id` - 매출 수정 (미마감만, 날씨 포함)
...
- 날짜, 점포, 매출액 입력 (필수)
- **날씨 선택** (☀️맑음, ☁️흐림, 🌧️비, ❄️눈, 🚫휴장)
- **숫자 키패드**로 매출액 입력 (빠르고 정확한 입력)
- 메모 입력 (선택)
- 같은 날짜에 같은 점포는 1건만 등록 가능

### 4. 매출 내역
- 전체 매출 내역 조회
- **당일 총매출 자동 계산** (순매출 + 부가세, 4개 점포 합계)
- **날씨 아이콘** 표시로 한눈에 영업 여건 확인
- 점포/상태/날짜 필터링
- 미마감 매출: 수정/삭제/마감 가능
- 마감된 매출: 관리자만 해제 가능
...
- **현재 상태**: 로컬 개발 완료 ✅
- **마지막 업데이트**: 2026-01-29 (추가사항 5가지 완료)
...
6. **날씨 기록**: 영업 환경 데이터 수집 (패턴 분석 가능)
7. **숫자 키패드**: 빠른 입력으로 업무 효율 향상
8. **당일 총매출**: 일일 마감 시 즉시 확인 가능
